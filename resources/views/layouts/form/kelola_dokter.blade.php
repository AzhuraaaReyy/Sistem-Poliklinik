        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align: center;">
                    @php
                    $alphabet = array_merge(range('a', 'z'), range('aa', 'zz')); // Menyediakan indeks hingga 'zz'
                    @endphp
                    @forelse($dokter->jadwal_periksa as $Index=> $jadwal)
                    <ul style="list-style-type: none; padding-left: 0;">
                        <li>{{$alphabet[$Index]}}. {{ $jadwal->hari }} - {{ $jadwal->jam_mulai }} s/d {{ $jadwal->jam_selesai }}</li>
                    </ul>
                    @empty
                    Belum Ada Jadwal
                    @endforelse
